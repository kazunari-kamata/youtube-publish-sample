...
初期値では通常動画だけを `export/update.mp4` に生成します。通常動画は30秒、Shorts は5秒です。Shorts だけ、または両方を生成したい場合は `BLENDER_GENERATE_TARGET` を指定します。
...
この Blender 生成では、`import/*.vrm` を glTF として Blender に読み込み、通常動画ではラジオ体操風に腕を開く、腕を上げる、体をひねる、深呼吸するといった簡易モーションを付けます。Shorts では要約用にモデルを回転させる動画を作成します。`import/*.unitypackage` がある場合は asset 数を読み取り、動画内のメタ情報として表示します。UnityPackage の prefab や scene を完全に再現する処理ではありません。
