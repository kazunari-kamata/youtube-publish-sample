...
`.github/workflows/publish-youtube.yml` は、次のどちらかで起動します。

- GitHub Actions 画面から手動実行する
- `output/update.mp4` の変更を `main` に push する
...
7. YouTube 側のタイトルを指定したい場合は、`youtube_title` に入力します。未指定の場合は日時付きタイトルが自動生成されます。
8. YouTube 側の説明文を指定したい場合は、`youtube_description` に入力します。未指定の場合は日時、repository、branch、commit、workflow run 番号を含む説明文が自動生成されます。
9. もう一度 `Run workflow` をクリックします。
10. workflow の実行が完了するまで待ちます。
11. 実行結果を開き、`アップロード結果を表示` step または job summary で YouTube の title、video ID、URL を確認します。

この方式では、手動で workflow を起動したときだけ YouTube へのアップロードが実行されます。
...
3. `output/update.mp4` を commit します。
...
5. GitHub Actions の `YouTube 更新動画の公開` が自動実行されます。
6. 実行結果の `アップロード結果を表示` step または job summary で YouTube の title、video ID、URL を確認します。

`output/update.mp4` が repository に存在する場合、Actions の `更新紹介動画を生成` step は既存ファイルを検出して生成をスキップします。その後、既存の `output/update.mp4` をアップロードします。

`output/update.mp4` の push で自動実行された場合も、YouTube の title / description を別途指定しなければ、日時、repository、branch、commit、workflow run 番号入りの値が自動で使われます。
