...
3. YouTube 側のタイトルや説明文を指定したい場合は、任意で metadata ファイルを作成します。不要な場合はこの手順を省略してください。
   ...
   `output/youtube-title.txt` は 1 行目だけが title として使われます。`output/youtube-description.md` は全文が description として使われます。どちらも未作成の場合は、日時、repository、branch、commit、workflow run 番号入りの値が自動で使われます。
