...
ローカルで作成した短い更新紹介動画を `main` に push し、GitHub Actions から YouTube Data API で限定公開アップロードするサンプルリポジトリです。

このサンプルは public repository として公開できるように、実在する個人情報、ローカル絶対パス、秘密情報を含めていません。

## できること

- `output/update.mp4` を `main` に push した場合は、GitHub Actions が自動で YouTube へアップロード
- GitHub Actions 内では動画を生成せず、push 済みの `output/update.mp4` だけをアップロード
...
- 任意で `output/youtube-title.txt` / `output/youtube-description.md` を commit し、YouTube の title / description を指定
...
`.github/workflows/publish-youtube.yml` は、`output/update.mp4` の変更を `main` に push した場合だけ起動します。

README や Python code だけを push しても YouTube アップロードは実行されません。GitHub Actions の `Run workflow` から手動実行する導線もありません。

この workflow は動画生成を行いません。ローカルや Codex CLI で作成して commit 済みの `output/update.mp4` を検証し、そのファイルを YouTube に限定公開アップロードします。
...
Codex CLI などでローカルに `output/update.mp4` を作成し、そのファイルを repository に commit して `main` に push すると、GitHub Actions が自動で YouTube に限定公開アップロードします。この方式では GitHub Actions 内で Codex CLI や ChatGPT 用の API token は使いません。GitHub Actions は、push 済みの動画ファイルを受け取って YouTube Data API でアップロードするだけです。
...
3. YouTube 側のタイトルや説明文を指定したい場合は、任意で metadata ファイルを作成します。不要な場合はこの手順を省略してください。
   ...
   `output/youtube-title.txt` は 1 行目だけが title として使われます。`output/youtube-description.md` は全文が description として使われます。どちらも未作成の場合は、アップロード日時入りの値が自動で使われます。

4. `output/update.mp4` を commit します。

   ```bash
   git add -f output/update.mp4
   git add output/youtube-title.txt output/youtube-description.md
   git commit -m "アップロード用動画を更新"
   ```

   metadata ファイルを作成していない場合、`git add output/youtube-title.txt output/youtube-description.md` は不要です。

   jj を使っている場合は、通常の jj の変更として `output/update.mp4` が含まれていることを確認してから commit 説明を付けます。
...
7. 実行結果の `アップロード結果を表示` step または job summary で YouTube の title、video ID、URL を確認します。

Actions は `output/update.mp4` を生成しません。push 済みの `output/update.mp4` が存在し、空ファイルでないことを確認してからアップロードします。
...
この hook は、`git push` 前に `output/update.mp4` が空ファイルでないこと、サイズが GitHub の通常制限に収まること、未 commit のまま残っていないことを確認します。未 commit の動画がある場合は push を止めるため、動画を commit し忘れたままアップロード workflow だけが走る事故を防げます。
...
初期設定では、タイトルに生成日時を含めた `萌え更新速報` の動画を作ります。動画内には、一般的なフリー素材風のアニメ立ち絵を参考にした完全オリジナルの萌え系女子キャラクター、星やハートの装飾、背景色の変化、移動や点滅などの動きを入れるよう Codex に依頼します。顔の輪郭と中心線を決めてから左右同じ高さに大きな瞳を配置するよう指定しているため、目の位置がズレにくくなります。
...
`output/update.mp4` は YouTube アップロード対象として repository に commit できます。それ以外の `output/*.mp4` は `.gitignore` の対象です。ローカル検証用の別名動画ファイルは repository に commit されません。
