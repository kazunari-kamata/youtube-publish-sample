...
- `output/update.mp4` と `output/update-shorts.mp4` を `main` に push した場合は、GitHub Actions が自動で YouTube へアップロード
...
- 任意で `output/youtube-title.txt` / `output/youtube-description.md` を commit し、通常動画の title / description を指定
- 任意で `output/youtube-shorts-title.txt` / `output/youtube-shorts-description.md` を commit し、Shorts 用動画の title / description を指定
...
`.github/workflows/publish-youtube.yml` は、`output/update.mp4` または `output/update-shorts.mp4` の変更を `main` に push した場合だけ起動します。

README や Python code だけを push しても YouTube アップロードは実行されません。GitHub Actions の `Run workflow` から手動実行する導線もありません。

この workflow は動画生成を行いません。ローカルや Codex CLI で作成して commit 済みの `output/update.mp4` と `output/update-shorts.mp4` を検証し、2本とも YouTube に限定公開アップロードします。
...
Codex CLI などでローカルに `output/update.mp4` と `output/update-shorts.mp4` を作成し、そのファイルを repository に commit して `main` に push すると、GitHub Actions が自動で YouTube に限定公開アップロードします。この方式では GitHub Actions 内で Codex CLI や ChatGPT 用の API token は使いません。GitHub Actions は、push 済みの動画ファイルを受け取って YouTube Data API でアップロードするだけです。
...
2. 動画ファイルを確認します。
   ...
   ```bash
   ffprobe -v error -show_entries format=duration,size -show_entries stream=width,height,nb_frames -of default=noprint_wrappers=1 output/update.mp4
   ffprobe -v error -show_entries format=duration,size -show_entries stream=width,height,nb_frames -of default=noprint_wrappers=1 output/update-shorts.mp4
   ```

3. YouTube 側のタイトルや説明文を指定したい場合は、任意で metadata ファイルを作成します。不要な場合はこの手順を省略してください。

   ```bash
   printf '%s\n' '萌え更新速報 2026-06-16 18:30' > output/youtube-title.txt
   cat > output/youtube-description.md <<'EOF'
   Codex CLI で作成した更新紹介動画です。
   今回の変更内容が分かるように、動画内にも日時を入れています。
   EOF

   printf '%s\n' '萌え更新速報 Shorts 2026-06-16 18:30 #Shorts' > output/youtube-shorts-title.txt
   cat > output/youtube-shorts-description.md <<'EOF'
   Codex CLI で作成した更新紹介動画の要約Shortsです。
   #Shorts
   EOF
   ```

   `output/youtube-title.txt` と `output/youtube-shorts-title.txt` は 1 行目だけが title として使われます。`output/youtube-description.md` と `output/youtube-shorts-description.md` は全文が description として使われます。未作成の場合は、アップロード日時入りの値が自動で使われます。

4. `output/update.mp4` と `output/update-shorts.mp4` を commit します。

   ```bash
   git add -f output/update.mp4 output/update-shorts.mp4
   git add output/youtube-title.txt output/youtube-description.md output/youtube-shorts-title.txt output/youtube-shorts-description.md
   git commit -m "アップロード用動画を更新"
   ```

   metadata ファイルを作成していない場合、2 行目の `git add` は不要です。

   jj を使っている場合は、通常の jj の変更として `output/update.mp4` と `output/update-shorts.mp4` が含まれていることを確認してから commit 説明を付けます。
...
7. 実行結果の `アップロード結果を表示` step または job summary で、通常動画と Shorts の title、video ID、URL を確認します。

Actions は動画ファイルを生成しません。push 済みの `output/update.mp4` と `output/update-shorts.mp4` が存在し、空ファイルでないことを確認してからアップロードします。
...
この hook は、`git push` 前に `output/update.mp4` と `output/update-shorts.mp4` が空ファイルでないこと、サイズが GitHub の通常制限に収まること、未 commit のまま残っていないことを確認します。未 commit の動画がある場合は push を止めるため、動画を commit し忘れたままアップロード workflow だけが走る事故を防げます。
...
初回の OAuth 同意だけはブラウザでの手動認可が必要です。GitHub Actions はブラウザで Google アカウントにログインできないため、初回認可まで完全自動化する構成にはしていません。認可後は、`output/update.mp4` を `main` に push したときに YouTube への限定公開アップロードを実行します。
...
このスクリプトは `codex exec` を呼び出し、`scripts/make_video.py` を使わずに ChatGPT / Codex の判断だけで `output/update.mp4` と `output/update-shorts.mp4` を作るよう依頼します。Codex は `ffmpeg` など利用可能なコマンドを選び、動画ファイルを直接生成します。Codex CLI が未インストール、または未ログインの場合は先に Codex CLI のセットアップが必要です。

初期設定では、通常動画は横長 16:9 の約 10 秒、Shorts 用動画は縦長 9:16 の約 5 秒で、タイトルに生成日時を含めた `萌え更新速報` の動画を作ります。動画内には、一般的なフリー素材風のアニメ立ち絵を参考にした完全オリジナルの萌え系女子キャラクター、星やハートの装飾、背景色の変化、移動や点滅などの動きを入れるよう Codex に依頼します。顔の輪郭と中心線を決めてから左右同じ高さに大きな瞳を配置するよう指定しているため、目の位置がズレにくくなります。

スクリプトは Codex CLI の実行後に `output/update.mp4` と `output/update-shorts.mp4` が存在し、空ファイルではないことを確認します。動画ファイルが作成されなかった場合はエラー終了します。
...
CODEX_SHORTS_OUTPUT="output/codex-update-shorts.mp4" \
...
`output/update.mp4` と `output/update-shorts.mp4` は YouTube アップロード対象として repository に commit できます。それ以外の `output/*.mp4` は `.gitignore` の対象です。ローカル検証用の別名動画ファイルは repository に commit されません。
