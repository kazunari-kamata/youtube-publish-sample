...
`.github/workflows/publish-youtube.yml` は、`export/update.mp4` または `export/update-shorts.mp4` の変更を `main` に push した場合だけ起動します。

README や Python code だけを push しても YouTube アップロードは実行されません。GitHub Actions の `Run workflow` から手動実行する導線もありません。

この workflow は動画生成を行いません。ローカルや Codex CLI で作成して commit 済みの `export/update.mp4` と `export/update-shorts.mp4` を検証し、2本とも YouTube に限定公開アップロードします。
...
`import/avator.png` が存在する場合、`scripts/create_video_with_codex.sh` / `scripts/create_video_with_codex.bat` はその画像を主要キャラクター素材として使うよう Codex CLI に依頼します。入力素材は `import/`、アップロード対象の完成動画は `export/` に分けています。
...
4. `export/update.mp4` と `export/update-shorts.mp4` を commit します。

   ```bash
   git add -f export/update.mp4 export/update-shorts.mp4
   git add export/youtube-title.txt export/youtube-description.md export/youtube-shorts-title.txt export/youtube-shorts-description.md
   git commit -m "アップロード用動画を更新"
   ```

   metadata ファイルを作成していない場合、2 行目の `git add` は不要です。

   jj を使っている場合は、通常の jj の変更として `export/update.mp4` と `export/update-shorts.mp4` が含まれていることを確認してから commit 説明を付けます。
...
Actions は動画ファイルを生成しません。push 済みの `export/update.mp4` と `export/update-shorts.mp4` が存在し、空ファイルでないことを確認してからアップロードします。
...
この hook は、`git push` 前に `export/update.mp4` と `export/update-shorts.mp4` が空ファイルでないこと、サイズが GitHub の通常制限に収まること、未 commit のまま残っていないことを確認します。未 commit の動画がある場合は push を止めるため、動画を commit し忘れたままアップロード workflow だけが走る事故を防げます。
...
このスクリプトは `codex exec` を呼び出し、`scripts/make_video.py` を使わずに ChatGPT / Codex の判断だけで `export/update.mp4` と `export/update-shorts.mp4` を作るよう依頼します。`import/avator.png` が存在する場合は、その画像を主要キャラクター素材として使い、背景、タイトル、装飾、ズーム、パン、フェード、軽い揺れなどの演出を追加して動画化するよう依頼します。Codex は `ffmpeg` など利用可能なコマンドを選び、動画ファイルを直接生成します。Codex CLI が未インストール、または未ログインの場合は先に Codex CLI のセットアップが必要です。
...
スクリプトは Codex CLI の実行後に `export/update.mp4` と `export/update-shorts.mp4` が存在し、空ファイルではないことを確認します。動画ファイルが作成されなかった場合はエラー終了します。
